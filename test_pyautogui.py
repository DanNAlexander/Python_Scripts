...
# Make the cmd the folder that this test_pyautogui.py is in:
scriptFolder = os.path.dirname(os.path.realpath(__file__))
os.chdir(scriptFolder) 

runningOnPython3 =sys.version_info[0] == 3
...
try:
    import pytweening
     ...
     assert False, "PyTweening must be installed to test"

try: 
    import pyscreeze
except: 
    assert False, "PyScreeze must be installed to test"

try:
    import pygetwindow
except: 
    assert False, "PyGetWindow must be installed to test"
    


### Immutable 2D Pointer Class with Arithmetic functions ###
